...
- 正式版：`MPLR-win-Installer.exe`
- Beta 版：`MPLR-win-beta-Installer.exe`

安装器会在运行内置 Velopack 安装包前让用户选择安装目录。软件内自动更新会继续更新当前安装目录，不会另行安装到默认目录导致出现两份。安装器默认不会开启开机自启动；开机自启动由用户在 MPLR 托盘菜单中手动开启或关闭。
...
GitHub Actions 会构建并发布自定义安装器和 Velopack 自动更新资产。旧的本地脚本仍可用于生成手动归档包：
