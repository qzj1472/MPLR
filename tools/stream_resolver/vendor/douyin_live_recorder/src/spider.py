...
async def get_douyin_web_stream_data(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None):
    ...
    try:
        ...
        try:
            ...
            room_data['anchor_name'] = user_data.get('nickname') or ''
            room_data['user'] = user_data
...
async def get_kuaishou_stream_data(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    result = {"type": 2, "is_live": False, "anchor_name": ""}
    ...
    author = play_list.get('author') or {}
    anchor_name = author.get('name') or author.get('userName') or author.get('user_name') or ''
    avatar = author.get('avatar') or author.get('avatarUrl') or author.get('headUrl') or ''
    result.update({"anchor_name": anchor_name, "avatar_thumb_url": avatar})
...
async def get_huya_app_stream_url(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    json_data = json.loads(json_str)
    profile_info = json_data['data'].get('profileInfo') or {}
    anchor_name = profile_info.get('nick') or ''
    avatar = profile_info.get('avatar180') or profile_info.get('avatar') or profile_info.get('avatarUrl') or profile_info.get('screenshot') or ''
    uid = str(profile_info.get('uid') or profile_info.get('yyid') or profile_info.get('profileRoom') or room_id)
    live_status = json_data['data']['realLiveStatus']
    live_title = json_data['data']['liveData']['introduction']
    if live_status != 'ON':
        return {'anchor_name': anchor_name, 'is_live': False, 'avatar_thumb_url': avatar, 'uid': uid}
    else:
...
async def get_bilibili_room_info(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    try:
        ...
        avatar = anchor_info['data']['info'].get('face', '')

        title = await get_bilibili_room_info_h5(url, proxy_addr, cookies)
        return {"anchor_name": anchor_name, "live_status": live_status, "room_url": url, "title": title, "avatar_thumb_url": avatar, "uid": uid}
    except Exception as e:
        print(e)
